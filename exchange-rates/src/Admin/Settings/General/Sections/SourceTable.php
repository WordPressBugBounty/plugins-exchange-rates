    ...
    public static function render()
    {
        ...
        foreach ($sources['data'] as $key => $value) {
            $table->add_row([
                ...
                esc_html(gmdate('Y-m-d H:i:s', $value['lastupdate'])),
            ]);
        }

       echo '<div class="table-responsive">' . wp_kses_post($table->generate()) . '</div><p><button type="button" data-id="exchange-rates-source-table-1" class="button show-more-table">' . esc_html__('Show More/Less', 'exchange-rates') . '</button></p>';
    }
}
