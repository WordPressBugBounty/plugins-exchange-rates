    ...
    protected function img($flag_type, $currency)
    {
        ...
        if ('emoji' === $flag_type) {
            ...
            $line = sprintf(
                ...
                $w,
                $h,
                sprintf('https://s.w.org/images/core/emoji/14.0.0/svg/%1$s.svg', esc_attr(strtolower($emoji[$currency['flag']]))),
                $currency['flag']
            );
            ...
            $line = sprintf(
                ...
                $w,
                $h,
                sprintf(plugin_dir_url($GLOBALS['dejurin_exchange_rates']->plugin_path).'assets/flags/'.$flag_type.'/%1$s.svg', esc_attr(strtolower($currency['flag']))),
                $currency['flag']
            );
        }
