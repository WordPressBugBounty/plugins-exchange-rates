jQuery(document).ready(function () {
  function showMoreLessTable() {
    jQuery(
      "#plugin-exchange-rates-admin table.exchange-rates-widgets-table"
    ).each(function () {
      var display = "none";
  ...
  jQuery('#plugin-exchange-rates-admin input[type="range"]').on(
    "input",
    function () {
      jQuery("#" + jQuery(this).attr("id") + "-show").html(jQuery(this).val());
    }
  );
  jQuery("#plugin-exchange-rates-admin button.show-more-table").each(
    function () {
      var total_table_tr = jQuery("#" + jQuery(this).data("id")).find(
        "tbody tr"
      ).length;
      var rows = jQuery("#" + jQuery(this).data("id")).data("rows");
      if (total_table_tr > rows) {
        jQuery(this).text(
          jQuery(this).text() + " (" + (total_table_tr - rows) + ")"
        );
      } else {
        jQuery(this).hide();
      }
  ...
  jQuery("#plugin-exchange-rates-admin button.show-more-table").on(
    "click",
    function () {
      var display = "hide";
  ...
  function dejurinExchangeRates_ShortcodeGenerator(_this) {
    ...
    if (currency_list !== "") {
      line +=
        'currency_list="' +
        currency_list.slice(0, currency_list.length - 1) +
        '"';
    }
    line += "]";

    if (
      jQuery(_this).find('select[multiple="multiple"] option:selected').length >
      0 ||
      jQuery(_this).find('select[multiple="multiple"]').length === 0
    ) {
      jQuery(_this).find("button").prop("disabled", false);
      jQuery(_this).find('textarea[name="shortcode-generator"]').val(line);
    }
  }
  jQuery(document).on(
    "input click",
    "#plugin-exchange-rates-admin",
    function (event) {
      var _this = this.closest("form");

      if (_this === null) {
        _this = jQuery("form[data-shortcode-generator]");
      }
      ...
      if (jQuery(event.target).prop("tagName") !== "TEXTAREA") {
        dejurinExchangeRates_ShortcodeGenerator(_this);
      }
    }
  );
  var _form = "form";
  ...
  jQuery("#plugin-exchange-rates-admin #shortcode-badge-color").wpColorPicker({
    change: function (event, ui) {
      dejurinExchangeRates_ShortcodeGenerator(_form);
      jQuery("#plugin-exchange-rates-admin #shortcode-badge-color").val(
        ui.color.toString()
      );
    },
    border: true,
